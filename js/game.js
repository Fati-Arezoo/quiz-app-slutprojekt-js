...
class Game {
  ...
  static game(level) {
    fetch(
      `https://opentdb.com/api.php?amount=3&difficulty=${level}&type=multiple`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  static loadCategory(currentIndex) {
    this.selectors.categoryText.textContent = this.state.questions[
      currentIndex
    ].category;
  }
  ...
  static loadAnswers(currentIndex) {
    ...
    choices[random].textContent = this.state.questions[
      currentIndex
    ].correctAnswer;
    wrongChoices.splice(random, 1);
    this.state.rightAnswerString = this.state.questions[
      currentIndex
    ].correctAnswer;
    for (let i = 0; i < wrongChoices.length; i++) {
      wrongChoices[i].textContent = this.state.questions[
        currentIndex
      ].wrongAnswers[i];
    }
  }
